...
local function notify_err(msg)
  vim.notify(msg, vim.log.levels.ERROR, { title = "claude_tmux" })
end
...
local function find_claude_pane()
  ...
  for _, line in ipairs(out) do
    ...
    if id and active == "0" and cmd and cmd:lower():match("claude") then
      return id
    end
  end
  return "{last}"
...
function M.send_ref(opts)
  ...
  vim.notify("Sent " .. token:gsub("%s+$", "") .. " → " .. pane, vim.log.levels.INFO, { title = "claude_tmux" })
end
